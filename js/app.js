 ...
 * V4.2 - Removed camera zoom on drag interaction.
 */
...
const CONFIG = {
    ...
    camera: {
        ...
        // dragZoomOut is no longer used but kept for potential future use
    },
    animation: {
        autoRotate: true,
...
class GlobeInteractionController {
    constructor(camera, earthGroup, renderer) {
        this.camera = camera; this.earthGroup = earthGroup; this.renderer = renderer;
        this.raycaster = new THREE.Raycaster(); this.mouse = new THREE.Vector2(); this.previousMouse = new THREE.Vector2();
        this.isDragging = false; this.rotationVelocity = 0; this.rotationDamping = 0.99; this.momentumThreshold = 0.0001;
        this.setupEvents();
    }
    setupEvents() {
    ...
    onMouseDown(event) {
        ...
        if (this.raycaster.intersectObject(world.earth, true).length > 0) {
            this.isDragging = true; this.rotationVelocity = 0;
            ...
            // *** FIX IS HERE ***: GSAP camera animation REMOVED
        }
    }
    onMouseMove(event) {
        this.updateMousePosition(event);
...
class FocusController {
    constructor(camera) { this.camera = camera; this.mouse = new THREE.Vector2(0, 0); this.currentFocus = CONFIG.postProcessing.focus; this.enabled = false; }
    onMouseMove(event) { if (!this.enabled) return; this.mouse.x = (event.clientX / window.innerWidth) * 2 - 1; this.mouse.y = -(event.clientY / window.innerHeight) * 2 + 1; }
    update() { if (!this.enabled || !postProcessing) return; const targetFocus = camera.position.z + (this.mouse.y * 3); this.currentFocus = THREE.MathUtils.lerp(this.currentFocus, targetFocus, 0.1); postProcessing.updateFocus(this.currentFocus); }
}
...
async function init() {
    ...
    try {
        ...
        postProcessing = null; document.getElementById('status-dofmode').textContent = 'ERROR';
    }
...
function animate() {
    ...
    if (DEBUG_STATE.logFrameInfo) console.log(`Frame: ${renderer.info.render.frame}, Tris: ${renderer.info.render.triangles}, Calls: ${renderer.info.render.calls}`);

    if (postProcessing) postProcessing.render(delta);
...
function handleKeyboardShortcuts(e) { if (e.key === 'd' && e.ctrlKey) { e.preventDefault(); document.body.classList.toggle('debug-mode'); return; } if (!document.body.classList.contains('debug-mode')) return; const keyActions = { 'r': resetParameters, 'f': toggleAutoFocus, 't': toggleTestObjects, 'd': () => postProcessing && postProcessing.debugDOF(), 's': logDetailedState, 'l': toggleFrameLogging, 'a': toggleAutoRotation, '1': () => focusOnObject(0), '2': focusOnEarth, '3': () => focusOnObject(3) }; if (keyActions[e.key.toLowerCase()]) keyActions[e.key.toLowerCase()](); }
...
function resetParameters() {
    const ppDefaults = { focus: 6.5, aperture: 8.0, maxblur: 0.015, saturation: 1.3, brightness: 1.0 };
    const earthDefaults = { relief: 1.0, displacement: 0.1, oceanShine: 0.5 };
...
function toggleAutoRotation() { if (!globeController) return; CONFIG.animation.autoRotate = !CONFIG.animation.autoRotate; window.dispatchEvent(new CustomEvent('autoRotationChange', { detail: { value: CONFIG.animation.autoRotate } })); }
...
function toggleTestObjects() { DEBUG_STATE.showTestObjects = !DEBUG_STATE.showTestObjects; world.testObjects.forEach(obj => obj.mesh.visible = DEBUG_STATE.showTestObjects); updateDebugStatus('testobjects', DEBUG_STATE.showTestObjects); }
...
function focusOnObject(index) { if (world.testObjects[index]) { const distance = camera.position.distanceTo(world.testObjects[index].mesh.position); updateDebugParameter('focus', distance); document.getElementById('focus').value = distance; document.getElementById('focus-input').value = distance.toFixed(2); } }
function focusOnEarth() { const dist = camera.position.distanceTo(world.earthGroup.position) - CONFIG.earth.radius; updateDebugParameter('focus', dist); document.getElementById('focus').value = dist; document.getElementById('focus-input').value = dist.toFixed(2); }
function logDetailedState() { console.log('=== DETAILED STATE ==='); console.log('CONFIG:', CONFIG); console.log('DEBUG:', DEBUG_STATE); console.log('Camera pos:', camera.position); console.log('Post-proc:', postProcessing ? postProcessing.params : 'N/A'); if (world.earth) { console.log('Material:', { normalScale: world.earth.material.normalScale.x, displacementScale: world.earth.material.userData.shader?.uniforms.uDisplacementScale.value, roughness: world.earth.material.roughness, metalness: world.earth.material.metalness }); } }
